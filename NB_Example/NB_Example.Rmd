...
---
The purpose of this code snippet is to demonstrate to the reader how a Naive Bayes (NB) classifier can be trained on given dataset using the R language. Note that building and deploying NB-enabled systems in industry settings would be more complicated than this simplified example (toy example) and such a discussion is out of the scope of this work. 

**The dataset:** We use the well-known Iris dataset (included with R) for this purpose. It contains four features (measurements) for 150 flowers representing three types of iris plants: Setosa, Versicolor and Virginica. 
...
dim(mydata) # check dimensions of dataset
...
head(dataset) # have a look at top data points in mydata
summary(dataset) # a summary of class distributions
```
**Creating training and validation datasets:** We're going to do an 80/20 partitioning for the training and validation sets. We use the createDataPartition function from the caret package for this purpose.
```{r}
...
testSet <- dataset[-tr_index,] # Select the remaining 20% of data for testSet
```
**Building a NB classifier:** Now we will train our NB classifier usinf the above trainSet. We will utilize e1071 package in R for this purpose. Note the priori probabilities when outputting the following lines of code. We get the same priori probabilities for all classes.
